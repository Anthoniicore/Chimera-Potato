...
<p align="center">
    A feature-rich mod for Halo Custom Edition and Halo Combat Evolved
</p>
...
* `chimera_block_zoom_blur [true/false]` - Get or set whether or not to disable the normal zoom blur.
...
* **... and many others...**
* **999 Additions:**
* **Pirate** - Invaluable feedback to any questions I had.
* **Exile** - Playtesting, finding bugs and providing feedback.
* **MasterMike** - Testing
...
* [Chimera 1.0 Source code](https://github.com/SnowyMouse/chimera)
* [Chimera -572 Source Code](https://github.com/Dwood15/Chimera)
* [Chimera -581 Source Code](https://github.com/ShellyHerself/Chimera581)
* [Chimera -600 Source Code](https://github.com/Devieth/Chimera600)
### Chimera FPS Boost (custom)

This fork includes `chimera_fps_boost [0-3]` for Halo CE 1.10.

* `0` - Restore the current-map visual changes made by the FPS profile.
* `1` - Disable anisotropic filtering, firing particles, normal zoom blur, and multitexture overlays when their optional signatures are available.
* `2` - Level 1 + remove BSP detail/bump texture dependencies and force the lowest shader lightmap detail for `shader_environment`/`shader_model`.
* `3` - Level 2 + remove BSP base texture dependencies and strip water texture dependencies/brightness.

The module **never modifies transparent shader classes** (`shader_transparent_chicago`, `shader_transparent_chicago_extended`, `shader_transparent_generic`, `shader_transparent_glass`, `shader_transparent_plasma`, etc.) and never disables the alpha render target. This is intentional so Active Camo keeps its normal liquid/refraction path. Player/weapon base textures are also preserved.

The tag changes are reversible on the current map and are reapplied after each map load while the profile remains active.
